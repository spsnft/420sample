...
import { createClientWithPrescription } from "@/app/staff/actions"
...
export function NewClientModal({ onClose }: NewClientModalProps) {
  ...
  const [firstVisitDate, setFirstVisitDate] = React.useState(today());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await createClientWithPrescription({
      name, phone, lineId, idNumber, firstVisitDate, pt33Number, issueDate, doctor,
    });
    ...
    onClose();
    router.push(`/staff/clients/${result.clientId}`);
  };
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        ...
        <h2 className="text-lg font-black uppercase tracking-tight text-brand-light mb-1">New Client</h2>
        <p className="text-[12px] text-brand-light/50 mb-4">Client details plus their first PT.33</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={lineId} onChange={(e) => setLineId(e.target.value)} className={inputClass} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className={labelClass}>First visit</label>
              <DateField required value={firstVisitDate} onChange={setFirstVisitDate} />
            </div>
          </div>

          <div className="pt-2 border-t border-white/10">
            <p className="text-[11px] font-black uppercase tracking-wide text-brand-secondary/80 mt-4 mb-1">PT.33</p>
          </div>
